  ...
  const handleImportFromFile = async () => {
    try {
      const selected = await openDialog({
        filters: [
          { name: "ishinex Agent", extensions: ["ishinex.json", "json"] },
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      const path = await save({
        defaultPath: `${agent.name.toLowerCase().replace(/\s+/g, "-")}.ishinex.json`,
        filters: [{ name: "ishinex Agent", extensions: ["ishinex.json"] }],
      });
