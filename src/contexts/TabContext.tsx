...
// const STORAGE_KEY = 'ishinex_tabs'; // No longer needed - persistence disabled
const MAX_TABS = 20;
...
export const TabProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    const loadTabs = async () => {
      ...
      const { tabs: savedTabs, activeTabId: savedActiveTabId } =
        TabPersistenceService.loadTabs();
        ...
        const restoredTabs = await Promise.all(
          savedTabs.map(async (tab) => {
            if (tab.type === "chat" && tab.sessionId) {
              ...
              const sessionData = SessionPersistenceService.loadSession(
                tab.sessionId,
              );
              if (sessionData) {
                // Create a Session object for the tab
                const session =
                  SessionPersistenceService.createSessionFromRestoreData(
                    sessionData,
                  );
                return {
                  ...tab,
  ...
  const addTab = useCallback(
    (
      tabData: Omit<Tab, "id" | "order" | "createdAt" | "updatedAt">,
    ): string => {
  ...
  const updateTab = useCallback((id: string, updates: Partial<Tab>) => {
    setTabs((prevTabs) =>
      prevTabs.map((tab) =>
        tab.id === id ? { ...tab, ...updates, updatedAt: new Date() } : tab,
      ),
    );
  }, []);
