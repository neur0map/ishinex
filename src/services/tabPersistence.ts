...
const STORAGE_KEY = "ishinex_tabs_v2";
const ACTIVE_TAB_KEY = "ishinex_active_tab_v2";
const PERSISTENCE_ENABLED_KEY = "ishinex_tab_persistence_enabled";
...
export class TabPersistenceService {
  ...
  static saveTabs(tabs: Tab[], activeTabId: string | null): void {
    ...
    try {
      ...
      const persistableTabs = tabs.filter((tab) => {
        ...
        if (tab.type === "create-agent" || tab.type === "import-agent")
          return false;
      ...
      const serializedTabs: SerializedTab[] = persistableTabs.map((tab) => ({
        ...
        createdAt:
          tab.createdAt instanceof Date
            ...
            : tab.createdAt,
        updatedAt:
          tab.updatedAt instanceof Date
            ? tab.updatedAt.toISOString()
            : tab.updatedAt,
      }));
      ...
      if (
        activeTabId &&
        persistableTabs.some((tab) => tab.id === activeTabId)
      ) {
  ...
  static loadTabs(): { tabs: Tab[]; activeTabId: string | null } {
    ...
    try {
      ...
      const activeTabId =
        savedActiveTabId &&
        orderedTabs.some((tab) => tab.id === savedActiveTabId)
          ? savedActiveTabId
