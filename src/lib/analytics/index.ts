  ...
  private sanitizeProperties(
    ...
    for (const [key, value] of Object.entries(properties)) {
      ...
      if (key.includes("path") || key.includes("file")) {
        sanitized[key] =
          typeof value === "string"
            ? sanitizers.sanitizeFilePath(value)
            : value;
      } else if (key.includes("project")) {
        sanitized[key] =
          typeof value === "string"
            ? sanitizers.sanitizeProjectPath(value)
            : value;
      } else if (key.includes("error") || key.includes("message")) {
        sanitized[key] =
          typeof value === "string"
            ? sanitizers.sanitizeErrorMessage(value)
            : value;
      } else if (key.includes("agent_name")) {
        sanitized[key] =
          typeof value === "string"
            ? sanitizers.sanitizeAgentName(value)
            : value;
      } else {
    ...
    events.forEach((event) => {
      if (typeof posthog !== "undefined" && posthog.capture) {
        posthog.capture(event.event, {
          ...
          $current_url: `ishinex://${event.properties?.screen_name || "unknown"}`,
        });
      }
    });
  }
