...
/// Register an external process for tracking (e.g., system processes, external tools)
#[tauri::command]
pub async fn register_external_process(
    registry: State<'_, crate::process::ProcessRegistryState>,
    run_id: i64,
    agent_id: i64,
    agent_name: String,
    ...
    registry.0.register_sidecar_process(
        run_id,
        agent_id,
        agent_name,
        pid,
...
pub async fn fetch_github_agents() -> Result<Vec<GitHubAgentFile>, String> {
    ...
    let url = "https://api.github.com/repos/neur0map/ishinex/contents/cc_agents";

    let response = client
        .get(url)
        .header("Accept", "application/vnd.github+json")
        .header("User-Agent", "ishinex-App")
        .send()
        .await
    ...
    // Filter only .ishinex.json agent files
    let agent_files: Vec<GitHubAgentFile> = api_files
        .into_iter()
        .filter(|f| f.name.ends_with(".ishinex.json") && f.file_type == "file")
        .filter_map(|f| {
